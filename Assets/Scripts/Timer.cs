...
public class Timer
{
    ...
    /// <typeparam name="T">Generic Monobehaviour to support running the coroutine</typeparam>
    /// <param name="_caller">Timer owner</param>
    public void StartLoopTimer<T>(T _caller) where T : MonoBehaviour
    {
        if (IsTimerDone == true)
        {
            IsTimerDone = false;
            IsTimerPaused = false;
            _caller.StartCoroutine(RunLoopTimer());
        }
        else
        {
            Debug.Log("Timer is not done, still on a loop!");
        }
    }
    ...
    /// <summary>
    /// Cancels timer and revokes action to be called
    /// </summary>
    /// <param name="_cancelAction"></param>
    public void CancelTimerAndAction()
    {
        OnTimerDone = null;
        IsTimerDone = true;
    }
    ...
    /// <summary>
    /// Sets delegate to call when the timer target is achieved
    /// </summary>
    /// <param name="_action"></param>
    public void SetTimerOnDoneAction(Action _action)
    {
        OnTimerDone = _action;
    }
    ...
    /// <summary>
    /// Updates current time until stop
    /// </summary>
    /// <returns></returns>
    private IEnumerator RunLoopTimer()
    {
        while (!IsTimerDone)
        {
            if (!IsTimerPaused)
